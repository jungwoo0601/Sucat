...
const InstructionImage = styled.div`
  width: 50%;
  height: 5%;
  background: url("/images/Game/GameInstruction1.png") no-repeat center center;
  background-size: contain;
  margin-top: 15vh;
  margin-bottom: 2vh;
  position: absolute;
    ...
    const fetchRankingData = async () => {
      try {
        const response = await axios.get(
          `${SERVER_URL}/api/v1/game/user/ranking?category=Fruit`,
          {
            headers: {
      ...
      <Container>
        ...
        {!showGame && (
          <>
            ...
            <ContentBox />
            <InstructionImage />
            <TitleButton>과일싫다냥</TitleButton>
            <InstructionText>
              냥이가 떨어지는 과일들을 피할 수 있게 도와주세요!
            </InstructionText>
