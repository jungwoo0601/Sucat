...
2) 시행일자 : 2018년 05월 01일 `,
  ...
  const handleNextButtonClick = async () => {
    if (allChecked) {
      try {
        console.log("server주소:", SERVER_URL);
        ...
        const response = await fetch(
          `${SERVER_URL}/api/v1/users/signup/next?${queryParams}`,
          {
            method: "GET",
          }
        );
  ...
  return (
    <>
      ...
      <Container>
        ...
        <CheckboxContainer>
          {["이용약관 동의(필수)", "개인정보 수집 및 이용(필수)"].map(
            (term, index) => (
              <CheckboxItem key={index}>
                ...
                <ViewButton onClick={() => openModal(index)}>보기</ViewButton>
              </CheckboxItem>
            )
          )}
        </CheckboxContainer>
